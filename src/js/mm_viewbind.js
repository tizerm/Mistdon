...
function createColumn(column) {
    ...
    // 閉じた状態のカラム
    let html = `
        <td id="${column.column_id}_closed" class="closed_col">
            <div class="col_action">
                <a class="__on_column_open" title="カラムを開く">
                    <img src="resources/ic_right.png" alt="カラムを開く"/>
                </a>
            </div>
            <h2>${column.label_head}<span></span></h2>
        </td>
    `;
    // 開いた状態のカラム
    html += `
        <td id="${column.column_id}" class="timeline">
            <div class="col_head">
                <h2>${column.label_head}</h2>
                <div class="col_action">
                    ...
                    <a class="__on_column_flex" title="可変幅ON/OFF"
                        ><img src="resources/${column.d_flex ? 'ic_flex_on' : 'ic_flex_off'}.png" alt="可変幅ON/OFF"/></a>
    ...
    // カラムの色と幅を変更
    $(`#columns>table #${column.column_id}>.col_head`).css("background-color", `#${column.col_color}`);
    $(`#columns>table #${column.column_id}_closed`).css("background-color", `#${column.col_color}`);
    $(`#columns>table #${column.column_id}`).css("width", column.d_flex ? "auto" : `${column.col_width}px`);

    // デフォルトで閉じる場合は表示を反転
    if (column.d_hide) {
        $(`#columns>table #${column.column_id}`).hide();
        $(`#columns>table #${column.column_id}_closed`).show();
    }
}
...
function getIntegratedPost(arg) {
    ...
    switch (arg.tl_account.platform) {
        case 'Mastodon': // Mastodon
            ...
            reblog_flg = arg.data.reblog ? true : false;
            break;
        case 'Misskey': // Misskey
            ...
            reblog_flg = arg.data.renote && !arg.data.text;
    ...
    return {
        ...
        'reblog_flg': reblog_flg,
        // ミュート情報を内包する(現状はBTRN非表示だけ)
        'muted': arg.timeline.exclude_reblog && reblog_flg,
        'post': arg.data
    };
}
...
function prependPost(arg, cache) {
    ...
    // 重複している投稿を除外する
    if (!cache.post_keyset.has(post.post_key) && !post.muted) {
        cache.post_keyset.add(post.post_key);
        ...
        ul.prepend(arg.bindFunc(post.post, post.from_address));
        // 未読カウンターを上げる
        ...
        // 追加アニメーション
        added.hide().show("slide", { direction: "up" }, 180);
    }
    if (post.binding_key == 'Mastodon-notification' || post.binding_key == 'Misskey-notification') {
        // 通知が来た場合は通知ウィンドウに追加
        // TODO: 通知表示に関してはまだ定型文が定まっていないのであとで書き直す
        prependNotification(`${post.from_address}に対して通知があります。`, false);
        window.accessApi.notification({
            title: post.from_address,
            body: `${post.from_address}に対して通知があります。`
        });
    }
}
...
function createIntegratedTimeline(array_json, bind_id, accounts) {
    ...
    // フッタのアカウントラベルに色を付ける
    $(`#columns>table #${bind_id}>ul>li>.post_footer>.from_address`).each(
        (index, elm) => $(elm).css("background-color", `#${accounts.get($(elm).attr("name")).acc_color}`));
}
...
function createTimelineMastLine(value, from_address) {
    ...
    if (viewdata.spoiler_text) {
        // CWテキストがある場合CWボタンを表示
        html += `
            <a class="expand_header label_cw">${viewdata.spoiler_text}</a>
    ...
    // 本文(絵文字を置換)
    html += `
                ${replaceEmojiMast(viewdata.content, viewdata.emojis)}
            </div>
        </div>
    `;
    if (viewdata.media_attachments.length > 0) {
        ...
        viewdata.media_attachments.forEach((media) => html += `
            <a href="${media.url}" name="${media.meta.original.aspect}" class="__on_media_expand">
                <img src="${media.preview_url}" class="media_preview"/>
            </a>
        `);
        html += `
                </div>
    ...
    if (from_address) {
        html += `
            <div class="from_address" name="${from_address}">From ${from_address}</div>
        `;
    }
    html += `
            </div>
...
function replaceEmojiMast(text, emojis) {
    ...
    emojis.forEach((emoji) => text = text.replace(
...
function createTimelineMskyLine(value, from_address) {
    ...
    if (viewdata.cw) {
        // CWテキストがある場合CWボタンを表示
        html += `
            <a class="expand_header label_cw">${viewdata.cw}</a>
            <div class="main_content cw_content">
        `;
    } else {
        // ない場合は普通にブロックを作る
    ...
    // 本文(絵文字を置換)
    html += `
                ${replaceEmojiMsky(viewdata.text, viewdata.emojis)}
            </div>
        </div>
    `;
    if (quote_flg) {
    ...
    if (viewdata.files.length > 0) {
        ...
        viewdata.files.forEach((media) => {
            ...
            html += `
                <a href="${media.url}" name="${aspect}" class="__on_media_expand">
                    <img src="${media.thumbnailUrl}" class="media_preview"/>
                </a>
            `;
        });
        html += `
...
function replaceEmojiMsky(text, emojis) {
    ...
    Object.keys(emojis).forEach((key) => text = text.replace(
        new RegExp(`:${key}:`, 'g'), `<img src="${emojis[key]}'" class="inline_emoji"/>`));
    return text;
}
...
function createContextMenuAccounts(accounts) {
    let html = '';
    accounts.forEach((v, k) => html += `
        <li name="${k}"><div>${v.username} - ${k}</div></li>
    `);
    return html;
}
